...
public class SneakerDetailResponsitory {
    ...
    public Integer addSneakerDetail(Model_SneakerDetail md) {
        ...
        try {
            Connection con = ConnectionJDBC.getConnection();
            String sql = "insert into SneakerDetail(sneaker_id,size_id,color_id,price,quantity,[status])\n"
                    + "values(?,?,?,?,?,?)";
            PreparedStatement prsm = con.prepareStatement(sql);
            prsm.setObject(1, md.getTenSP().getProduct_id());
            prsm.setObject(2, md.getKichCo().getId_Size());
            prsm.setObject(3, md.getMauSac().getColor_id());
            prsm.setObject(4, md.getGiaSP());
            prsm.setObject(5, md.getSoLuong());
            prsm.setObject(6, md.getTrangThai());
